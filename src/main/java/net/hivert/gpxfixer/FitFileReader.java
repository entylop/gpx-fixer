...
public class FitFileReader {
    ...
    private MesgListener genericListener = null;
    ...
    public void setGenericListener(MesgListener genericListener) {
    ...
    /**
     * Override the default read behaviour by skipping header decode. CRC checking is not possible
     * since the datasize is unknown. Decode continues until EOF is encountered or a decode error
     * occurs. May only be called prior to calling read(). Can be combined with incompleteStream()
     * option.
     */
    public void skipHeader() {
        ...
        if (in != null) {
            throw new RuntimeException("Can't set skipHeader option after Decode started!");
        }
        // Skip header decode
        state = STATE.RECORD;
    ...
    /**
     * Override the default read behaviour allowing decode of partial streams. If EOF is encountered
     * no exception is raised. Caller may choose to call resume() possibly after more bytes have
     * arrived in the stream. May only be set prior to first calling read().
     *
     */
    ...
    public boolean resume() {
        ...
        pause = false;
        FitFileReader.RETURN decodeReturn = RETURN.CONTINUE;
        ...
        try {
            while ((data = in.read()) >= 0) {
                ...
                default:
                    throw new RuntimeException("FIT decode error: " + decodeReturn);
                }
            }
        ...
        if ((instreamIsComplete == true) && (fileBytesLeft != DECODE_DATA_RECORDS_ONLY)) {
            ...
            throw new RuntimeException("FIT decode error: Unexpected end of input stream.");
        }
    ...
    static public boolean isFit(final InputStream in) {
        FitFileReader decode = new FitFileReader();
        int data;
    ...
    public RETURN read(byte data) {
            ...
            if (fileBytesLeft == 1) { // CRC low byte.
                if (state != STATE.RECORD)
                    throw new RuntimeException(
                            "FIT decode error: Decoder not in correct state after last data byte in file.  Check message definitions.");

                return RETURN.CONTINUE; // Next byte.
            } else if (fileBytesLeft == 0) { // CRC high byte.
                if (crc != 0)
                    throw new RuntimeException("FIT decode error: File CRC failed.");
        ...
        case FILE_HDR:
            ...
            case 1:
                if ((data & Fit.PROTOCOL_VERSION_MAJOR_MASK) > (Fit.PROTOCOL_VERSION_MAJOR << Fit.PROTOCOL_VERSION_MAJOR_SHIFT))
                    throw new RuntimeException("FIT decode error: Protocol version "
                            + ((data & Fit.PROTOCOL_VERSION_MAJOR_MASK) >> Fit.PROTOCOL_VERSION_MAJOR_SHIFT) + "."
                            + (data & Fit.PROTOCOL_VERSION_MINOR_MASK) + " not supported.  Must be "
                            + Fit.PROTOCOL_VERSION_MAJOR + ".15 or earlier.");
                break;
            case 4:
            ...
            case 8:
                if (data != '.')
                    throw new RuntimeException(
                            "FIT decode error: File is not FIT format.  Check file header data type.");
                break;
            case 9:
                if (data != 'F')
                    throw new RuntimeException(
            ...
            if (fileBytesLeft > 1) {
                if ((data & Fit.HDR_TIME_REC_BIT) != 0) {
                    ...
                    if (localMesgDefs[localMesgIndex] == null)
                        throw new RuntimeException(
                                "FIT decode error: Missing message definition for local message number "
                                        + localMesgIndex + ".");
        ...
        case MESG_NUM_1:
            ...
            if (localMesgDefs[localMesgIndex].arch == Fit.ARCH_ENDIAN_BIG) {
                localMesgDefs[localMesgIndex].num = (localMesgDefs[localMesgIndex].num >> 8)
                        | ((localMesgDefs[localMesgIndex].num & 0xFF) << 8);
            } else if (localMesgDefs[localMesgIndex].arch != Fit.ARCH_ENDIAN_LITTLE) {
                throw new RuntimeException("FIT decode error: Endian " + localMesgDefs[localMesgIndex].arch
                        + " not supported.");
            }
        ...
        case FIELD_DATA: {
            ...
            if (fieldBytesLeft == 0) {
                ...
                if ((fieldDef.type & Fit.BASE_TYPE_NUM_MASK) < Fit.BASE_TYPES) { // Ignore field if
                                                                                 // base type not
                                                                                 // supported.
                    ...
                    if (field != null) {
                        // For unknown fields the factory will create a field with the type set to
                        // 0, i.e. enum
                        ...
                        // Allow messages containing the accumulated field to set the accumulated
                        // value
                    ...
                    for (int i = 0; i < mesg.fields.size(); i++) {
                        ...
                        if (activeSubfield == Fit.SUBFIELD_INDEX_MAIN_FIELD) {
                            ...
                            if (mesg.fields.get(i).subFields.get(activeSubfield).components.size() > 0) {
                                ...
                                expandComponents(mesg.fields.get(i),
    ...
    protected void expandComponents(Field containingField, ArrayList<FieldComponent> componentList) {
        ...
        for (i = 0; i < componentList.size(); i++) {
            ...
            if (component.fieldNum != Fit.FIELD_NUM_INVALID) {
                ...
                // If the component field itself has *one* component apply the scale and offset of
                // the componentField's
                // (nested) component
                if (componentField.components.size() == 1) {
                    value = (((bitsValue / component.scale) - component.offset) + componentField.components.get(0).offset)
                ...
                // field data must already be encoded. Add elements to it until we have added
                // bitsvalue
                ...
                // componentField is an ordinary field, apply scale and offset as usual
                    ...
                    value = (((bitsValue / component.scale) - component.offset) + componentField.offset)
